...
def get_audio_features_artist_top_tracks(artist_list):
    ...
    for artist_name in artist_list:
        ...
        for i in range(0, len(track_uris), 50):
            ...
            artist_audio_features.setdefault(artist_name, []).extend(sp.audio_features(batch))
...
def get_audio_features_artist_all_tracks(artist_list):
    ...
    artist_audio_features = {} # Initialize an empty dictionary to store audio features for each artist
    ...
    artist_audio_feats = {} # Initialize an empty dictionary to store the mean audio features for each artist's tracks
    ...
    for artist_name, track_features in artist_audio_features.items():
        artist_track_feats = [] # Initialize an empty list to store the audio features for each track
        for track_feat in track_features:
            artist_track_feats.append([track_feat.get(key) for key in keys]) # Append the specified keys from the audio features dictionary to the artist_track_feats list
            artist_audio_feats[artist_name] = np.mean(np.array(artist_track_feats),axis=0) # Calculate the mean audio features for the artist's tracks and store them in the artist_audio_feats dictionary
...
def plot_artist_ranking(df_similarity_scores_from_artist, image_dict):
    ...
    # Create the plot
    fig, axs = plt.subplots(1, len(df_similarity_scores_from_artist) + 1, figsize=(50, 50))
    axs[0].imshow(img)
    axs[0].axis('off')
    axs[0].set_title("COACHELLA\nHere are the artist\nmost similar to\n{}".format(df_similarity_scores_from_artist.columns.values[1]),
                      fontname='Luminari',
                      fontsize=50,
                      fontstyle='italic')
...
def get_top_artist_recommendations(df_user, df_fest, n):
    ...
    for artist in df_result.index.values:
        artist_scores[artist] = df_result.T[artist].sort_values(ascending=False).head(n).reset_index().rename(columns={'index': 'artist'})
...
def plot_all_recommended_artists(artist_cosim_scores,artist_images):
    ...
    for i, artist in enumerate(rec_artist):
        ...
        axs[row, col].set_title(f"{artist}",
                                 fontname='Luminari',
                                 fontsize=10,
                                 fontstyle='italic')
        color = '#e3dac9'
...
# csp = spotipy.Spotify(auth_manager=SpotifyOAuth(scope=scope, client_id=client_id, client_secret=client_secret, redirect_uri='http://localhost:8888/callback'))
...
# results=csp.current_user_playlists(limit=50)
# playlists=[]
# for i in results['items']:
#     playlists.append({i['name'],i['uri']})
...
#     duh = get_cosim_artist_df(req_name, df_artist, 5)
